  ...
  React.useLayoutEffect(() => {
    ...
    try {
      ...
      if (s && Number.isFinite(s.x) && Number.isFinite(s.y) && Number.isFinite(s.scale)) {
        // Sanity check: if the world is parked entirely outside the viewport
        // (e.g. content was at y=0 but a stale save has y=-6000), the canvas
        // looks blank on load and the pagehide flush re-saves the bad value
        // on every reload — impossible to escape without dev tools. Validate
        // against the world's own bounds and discard if no overlap.
        const vp = vpRef.current;
        const world = worldRef.current;
        if (vp && world) {
          const vw = vp.clientWidth, vh = vp.clientHeight;
          // Read world's natural (untransformed) size from its layout box.
          // offsetWidth/Height isn't affected by our translate3d+scale because
          // those are transforms, not layout.
          const ww = world.offsetWidth * s.scale;
          const wh = world.offsetHeight * s.scale;
          const visible = s.x + ww > 0 && s.y + wh > 0 && s.x < vw && s.y < vh;
          if (!visible) {
            // bail — keep the default {0,0,1} centered transform
            return;
          }
        }
